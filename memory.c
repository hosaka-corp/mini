...
void _ahb_flush_to(enum AHBDEV dev)
{
	...
	//NOTE: 0x0d8b000x, not 0x0d8b400x!
	u32 val = _mc_read32(0x0d8b0008);
	if (!(val & mask))
	{
		switch(dev) {
		...
		case AHB_SHA1:
		case AHB_SDHC:
			while ((read32(HW_SPARE1_BOOT0) & 0xF) == 9)
			...
			write32(0x0d8b0008, _mc_read32(0x0d8b0008) & (~mask));
			write32(0x0d8b0008, _mc_read32(0x0d8b0008) | mask);
...
void ahb_flush_from(enum AHBDEV dev)
{
	...
	// Wait for the AHB to acknowledge our request
	for (i=0; i < 1000000; i++)
	{
...
void mem_setswap(int enable)
{
	...
	// Disable SRAM mirror
	if ((d & SRAM_MIRROR) && !enable)
		write32(HW_MEMMIRR, d & ~SRAM_MIRROR);
...
u32 dma_addr(void *p)
{
	...
	case 0x0d4:
	case 0x0dc:
		if (read32(HW_MEMMIRR) & SRAM_MIRROR) {
			addr ^= 0x10000;
		}
		addr &= 0x0001FFFF;
		addr |= 0x0d400000;
